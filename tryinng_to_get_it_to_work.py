...
def menu(event):
    ...
    if click(event, button_tutorial):
        canvas.create_text(w // 2, h // 0.5, font=("Masquerade", 40), fill="lime", text="типо обучалка")
    elif click(event, button_stage):
        canvas.create_text(w // 2, h // 0.5, font=("Masquerade", 40), fill="lime", text="типо уровень")
    elif click(event, button_boss):
        canvas.create_text(w // 2, h // 0.5, font=("Masquerade", 40), fill="lime", text="типо босс")
